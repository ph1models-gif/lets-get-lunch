...
- v2 (in progress): dropped `scale=2` → non-retina (57KB, confirmed via curl) AND dropped `fetchPriority="high"`, shipped as a single isolated change to measure before touching anything else. If LCP is still bad, next steps to try one at a time: re-add `fetchPriority="high"` on the 57KB image (the static map is *supposed* to be the LCP element per the Job 0 audit, so it may need it once it's not competing at 133KB), then try `format=jpg` on the Static Maps URL to shrink further.
- **If none of that recovers close to the ~58 pre-facade score, the honest move is to revert this entire facade and go back to the plain "Loading map…" skeleton** — that was a fine, deliberately-chosen place to stop (see DECISION above), and isn't worth an open-ended tuning cycle to save.
