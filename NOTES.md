...
- v3 (in progress): re-added `fetchPriority="high"` on the same 57KB (non-retina) image — the static map is *supposed* to be the LCP element per the Job 0 audit, so testing whether it needs the priority hint once it's not competing at 133KB. Single isolated variable again. If this doesn't recover close to ~58, next lever is `format=jpg` on the Static Maps URL; if that still doesn't get there, revert the facade entirely (see below).
