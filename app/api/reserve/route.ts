...
export async function POST(req: NextRequest) {
  try {
    const { restaurant_id, restaurant_name, restaurant_email, name, contact, party_size, preferred_time, note } = await req.json()
    ...
    if (restaurant_email) {
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'Let\'s Get Lunch <onboarding@resend.dev>',
          to: restaurant_email,
          subject: `New lunch reservation — ${name}, party of ${party_size}`,
          html: `
            <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:24px">
              <h1 style="color:#4A9FD5;font-size:24px;margin-bottom:8px">New reservation! 🎉</h1>
              <p style="color:#444;font-size:16px">You have a new lunch reservation from Let's Get Lunch.</p>

              <div style="background:#EEF6FC;border-radius:12px;padding:24px;margin:24px 0">
                <p style="margin:4px 0;font-size:15px"><strong>Guest:</strong> ${name}</p>
                <p style="margin:4px 0;font-size:15px"><strong>Contact:</strong> ${contact}</p>
                <p style="margin:4px 0;font-size:15px"><strong>Time:</strong> ${preferred_time}</p>
                <p style="margin:4px 0;font-size:15px"><strong>Party size:</strong> ${party_size} ${party_size === 1 ? 'person' : 'people'}</p>
                ${note ? `<p style="margin:4px 0;font-size:15px"><strong>Note:</strong> ${note}</p>` : ''}
                <p style="margin:16px 0 4px;font-size:13px;color:#666">Reservation code (guest will show this)</p>
                <p style="color:#4A9FD5;font-size:28px;font-weight:700;letter-spacing:4px;margin:0">${code}</p>
              </div>

              <p style="color:#888;font-size:13px">Powered by Let's Get Lunch · letsgetlunch.com</p>
            </div>
          `,
        }),
      })
